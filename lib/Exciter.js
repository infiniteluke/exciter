...
class Exciter {
  ...
  */
   ...
   *   The name of the partitionKey attribute. This is used to ensure an
   *   existing record does not already exist.
   *
   * @return {Promise}
   *   Resolves when the document has been written to DynamoDB, rejects if there
   *   was an error.
   */
  create(data, table, partitionKey) {
    return this.put(data, table, partitionKey, true);
  }
   ...
   *   An object containing partitionKey and sortKey key/value properties.
   * @param {String} table
   *   The table in which to save the document.
   *
   * @return {Promise}
   ...
   *   The table in which to save the document.
   * @param {String} partitionKey
   *   The name of the partitionKey attribute. This is used when specifying
   *   createOnly to ensure an existing record does not already exist.
   ...
   *
   * @param {Object} primaryKey
   *   An object containing key value pairs for the DynamoDB partitionKey and
   *   optionally sortKey.
   * @param {String} table
  ...
  static normalizeExpressionAttribute(rawValue, name) {
    ...
    // Fail when value is null or undefined.
    if (_.isNil(attribute.value)) {
   ...
   * @param {String} substituionChar
   *   What string to prepend to keys of the object. ex: `#` or `:`
   *
   * @return {Object}
   *   A DynamoDB expression attribute values/names object.
   */
  static buildExpressionPlaceholders(attributes, substituionChar) {
    return attributes.reduce((values, attribute) => {
      // Handle names.
      if (substituionChar === '#') {
        if (_.has(attribute, 'path')) {
          attribute.path.split('.').forEach((fragment) => {
            values[`${substituionChar}${attribute.name}_${fragment}`] = String(fragment);
          });
        }
        else {
          values[`${substituionChar}${attribute.name}`] = String(attribute.name);
        }
      }
      // Handle array values.
      else if (_.isArray(attribute.value)) {
        attribute.value.forEach((val, i) => {
          values[`${substituionChar}${attribute.name}${i}`] = val;
        });
      }
      // Handle simple values.
      else {
        values[`${substituionChar}${attribute.name}`] = attribute.value;
      }
